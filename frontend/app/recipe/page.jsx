...
import {
  ArrowLeft, Clock, Users, ChefHat, Flame, Lightbulb, Bookmark, BookmarkCheck, Loader2, AlertCircle, CheckCircle2, Download,
} from "lucide-react";
            ...
            <p className="text-lg text-stone-600 mb-6 font-light">
              {(() => {
                ...
                if (Array.isArray(desc)) {
                  const firstParagraph = desc.find((block) => block.type === "paragraph");
                  if (firstParagraph && Array.isArray(firstParagraph.children)) {
                    return firstParagraph.children.map((child) => child.text || "").join("");
                  }
                }
                return "";
              })()}
                ...
                <div key={category} className="mb-6 last:mb-0">
                  ...
                  <ul className="space-y-2">
                    {items.map((ingredient, i) => (
                      <li
                        key={i}
                        className="flex justify-between items-start gap-4 text-stone-700 py-3 border-b border-stone-100 last:border-0"
                      >
                        <span className="flex-1 break-words min-w-0 pr-2 leading-snug">
                          {ingredient.item}
                        </span>
                        <span className="font-bold text-orange-600 text-sm whitespace-normal text-right shrink-0 max-w-[40%] leading-snug">
                          {ingredient.amount}
                        </span>
                      </li>
                    ))}
